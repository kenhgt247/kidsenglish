...
import { Volume2, CloudRain, Sun } from 'lucide-react';
...
const RainbowColors: React.FC = () => {
  ...
  return (
    <div className="h-full bg-sky-300 overflow-hidden flex flex-col p-4 relative">
          ...
          {COLORS.map((c, i) => {
            ...
            return (
              <motion.div
                key={c.id}
                initial={{ opacity: 0 }}
                animate={{ opacity: isUnlocked ? 1 : 0.1, scale: isUnlocked ? 1 : 0.95 }}
                className="absolute left-1/2 bottom-0 -translate-x-1/2 rounded-t-full border-[12px] md:border-[24px]"
                style={{ 
                  width: `${size}%`, 
                  height: `${size * 2}%`, 
                  borderColor: isUnlocked ? c.hex : '#FFFFFF',
                  zIndex: 50 - i
                }}
              />
            );
      ...
      <AnimatePresence>
        {bubbles.map(b => (
          <motion.button
            ...
            onClick={() => handlePop(b)}
            className="absolute w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-white shadow-xl z-20"
            style={{ left: `${b.x}%`, backgroundColor: `${b.color.hex}88` }}
          >
            <div className="absolute top-4 left-6 w-6 h-3 bg-white/40 rounded-full" />
          </motion.button>
        ))}
      </AnimatePresence>
      ...
      {isVictory && (
        <div className="fixed inset-0 bg-sky-500/90 backdrop-blur-xl z-[150] flex items-center justify-center p-6 text-center">
          <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="bg-white p-12 rounded-[4rem] shadow-2xl max-w-sm w-full border-8 border-white flex flex-col items-center gap-8">
            ...
            <h2 className="text-4xl font-black text-sky-900 uppercase">Rainbow Master!</h2>
            <button onClick={() => navigate('/map')} className="w-full bg-sky-600 text-white text-3xl font-black py-6 rounded-3xl shadow-[0_10px_0_0_#0284C7] active:translate-y-1 transition-all">
              LET'S GO!
            </button>
          </motion.div>
        </div>
